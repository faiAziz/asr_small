...
def create_dataset(tokenizer, ascending, name, path, bucketing, batch_size,
    ...
    elif name.lower() == 'quran':
        from corpus.quran import QuranDataset as Dataset
    else:
...
def create_textset(tokenizer, train_split, dev_split, name, path, bucketing, batch_size):
    ...
    if name.lower() == "librispeech":
        ...
        from corpus.quran import QuranTextDataset as Dataset
    else:
        raise NotImplementedError
...
def load_dataset(n_jobs, use_gpu, pin_memory, ascending, corpus, audio, text):
    ...
    #print('len of train before: '+str(len(tr_set)))
    ...
    #check dataset
    #print('len of train dataset: '+str(len(tr_set)))
    #print('example: ')
